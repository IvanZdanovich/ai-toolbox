...
class AIService {
  ...
  async processWithLlama(prompt) {
    if (!this.settings.apiKey) {
      throw new Error('Llama API key not configured. Please add your API key in settings.');
    }
  ...
  async processWithGrok(prompt) {
    if (!this.settings.apiKey) {
      throw new Error('Grok API key not configured. Please add your API key in settings.');
    }
  ...
  async validateApiKey(provider, apiKey) {
    if (provider === AI_PROVIDERS.MOCK) {
      return { valid: true, message: 'Mock provider does not require API key validation' };
    }
