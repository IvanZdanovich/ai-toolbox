...
export function validateTemplate(template) {
  ...
  if (!template.name || template.name.trim().length === 0) {
    ...
    errors.push(
      `Template name must be ${LIMITS.MAX_TEMPLATE_NAME_LENGTH} characters or less`
    );
  }

  if (
    template.description &&
    template.description.length > LIMITS.MAX_TEMPLATE_DESCRIPTION_LENGTH
  ) {
    errors.push(
      `Template description must be ${LIMITS.MAX_TEMPLATE_DESCRIPTION_LENGTH} characters or less`
    );
  }
  ...
  if (!template.prompt || template.prompt.trim().length === 0) {
    ...
    errors.push(
      `Template prompt must be ${LIMITS.MAX_TEMPLATE_PROMPT_LENGTH} characters or less`
    );
  }
