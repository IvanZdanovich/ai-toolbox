---
paths:
  - 'reqs/integration/**/*.integration.test.js'
  - 'reqs/integration-examples/**/*.examples.js'
---

# Integration specs

INTEGRATION_FILENAME: names the file for the primary module's source base name plus `.integration.test.js` and files it at `reqs/integration/<primary-module-app-path>/` — `chrome-extension/background/background.js` is verified by `reqs/integration/background/background.integration.test.js` — and backs it, when it needs data, with `reqs/integration-examples/<same-path>/<name>.examples.js` — otherwise the suffix is the only thing distinguishing this level from `reqs/unit/`'s bare `.test.js` at the same mirrored path, and dropping it collides the two.
