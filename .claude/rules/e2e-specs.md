...
E2E_FILENAME: names the file `reqs/e2e/<flow>.e2e.spec.js` — the `.e2e.` infix is what separates it from `reqs/integration/`'s `.integration.spec.js` and `reqs/unit/`'s bare `.spec.js` once both are flattened into one Vitest run, and `npm run test:e2e` selects the directory — and backs it, when it needs data, with `reqs/e2e-examples/<flow>.examples.js` — otherwise a plain `.spec.js` here reads as a unit spec that wandered up.
...
E2E_IS_FLAT: keeps `reqs/e2e/` free of a mirrored sub-tree and names the file for the flow (`workflows.e2e.spec.js`) over a module, because a flow crosses modules and has no single app path to mirror — otherwise the case ends up filed under whichever module it happened to start in.
...
E2E_FLAT_CHECK: every case sits directly under `reqs/e2e/` and is named `<flow>.e2e.spec.js`; none sits under `reqs/integration/` or carries an integration suffix.
