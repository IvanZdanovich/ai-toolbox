...
SMOKE_EARNS_THE_BROWSER: covers only what jsdom cannot and what is about the _install_ rather than about a user — Chrome accepting `manifest.json`, a module specifier resolving against Chrome's loader rather than Vitest's, the service worker registering, a page booting with no console error — and sends a user flow to `reqs/e2e/`, where Playwright drives the shipped UI, and anything jsdom could answer further down — otherwise the two browser levels grow the same suite twice.
...
SMOKE_FILENAME: names the file `reqs/browser/<flow>.smoke.js`, flat and named for the flow — `extension-loads.smoke.js`, `template-run.smoke.js` — never `.spec.js`, the suffix Vitest and Playwright collect between them — otherwise the file either drops out of every runner or lands in one that cannot execute it.
