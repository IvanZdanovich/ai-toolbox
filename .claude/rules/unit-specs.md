...
UNIT_MIRRORS_SOURCE: mirrors `chrome-extension/<path>` at `reqs/unit/<path>` — same relative folder path, one spec file per source item, named for the source file with its `.js` swapped for `.spec.js` and no level infix: `chrome-extension/shared/storage.js` → `reqs/unit/shared/storage.spec.js`, and a directory module takes its directory's name over `index.spec.js`, as `shared/components/editor-tab/index.js` → `reqs/unit/shared/components/editor-tab.spec.js` does — otherwise a renamed or moved source file leaves its spec orphaned and undiscoverable.
...
UNIT_EXAMPLE_IS_NAMED: exports each instance as a named constant a case can read as a sentence — `import { emailTemplate } from '../../unit-examples/shared/template.examples.js'` — over a namespace a case indexes into, so the spec body states the scenario rather than a path through a fixture pile.
UNIT_EXAMPLE_SPLIT_PENDING: adds no fifth subject to `reqs/unit-examples/shared/test-data.examples.js`, which serves four (templates, history, settings, AI responses) through a single `fixtures` namespace reached as `fixtures.templates.email` — splitting it by subject into named exports is what `UNIT_EXAMPLE_IS_NAMED` asks for and is blocked on 112 call sites, so it is the one standing exception; a new subject gets its own `<subject>.examples.js` with named exports rather than a fifth key here — otherwise the file it is already too late to split cheaply grows further.
