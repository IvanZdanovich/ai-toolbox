...
description: Use when structuring a new component/module or restructuring an existing one along with its tests and docs, when making and recording a pivot-level architectural decision (including how a boundary should be enforced) and its motivation, or when a conversation surfaces a business rule that should be captured as an executable constraint/example/spec requirement. Routine or low-level technical decisions are never recorded — only pivots and business rules are.
---
...
PIVOT_OR_RULE_ONLY: record a decision or requirement only when it is a pivot (it redirects the system's architecture or forecloses a direction) or a business rule (a constraint the domain/business imposes, not one the code merely happens to implement) — a routine technical choice (library pick with no lock-in, naming, file layout, a boilerplate pattern, a refactor with no external consequence) is never recorded, no matter how much discussion it took — otherwise the log fills with technical trivia that drowns the decisions that actually matter
DECISION_BAR: record a decision only when it clears PIVOT_OR_RULE_ONLY, is non-obvious, would cost real time or cause a defect if unknown, and would recur for the next person touching this area — otherwise decision logs fill with routine choices nobody needs to look up
SCOPE_MATCH: file a doc, decision, or requirement at the smallest scope that fully contains it — module doc for one module, area doc for a recurring pattern, decision record for a one-time cross-cutting pivot, requirement file trio for a business-rule constraint — otherwise a local trap hides in a global doc, a systemic decision hides in one module, or a granular constraint clutters a decision record
...
BAR_CHECK: each recorded decision is a pivot or business rule (PIVOT_OR_RULE_ONLY) and clears the non-obvious + costly + recurring bar; routine technical choices are left out
